function [Sigma,Alpsup,Alpsup2,w0,w02,history,delta_k,delta_k2,objective] = mklsvm(K,yapp,C,option,verbose)
...
switch option.algo
    case 'svmclass'
        option.train='p';
        [nsvm, pi,w0,obj,delta_k] = EpsilonTSVM4MKLnew(kerneloption.matrix,yapp,option,C,alphainit);        
        option.train='n';
        [nsvm2, pi2,w02,obj2,delta_k2] = EpsilonTSVM4MKLnew(kerneloption.matrix,yapp,option,C,alphainit);
        [grad] = gradsvmclass4MKL(K,pi,pi2,C,yapp,option);
...
while loop & nloopmax >0 ;
    ...
    history.obj2=[history.obj2 obj2];
...
%     [Sigma,Alpsup,w0,pos,obj] = mklsvmupdate(K,Sigma,pos,Alpsup,w0,C,yapp,grad,obj,option);
    ...
    [Sigma,Alpsup,Alpsup2,w0,w02,obj,delta_k,delta_k2] = mklsvmupdate(K,Sigma,pi,pi2,C,yapp,grad,obj,option);
%     if abs(Sigma-Sigmaold)<0.000001
%         loop=0;
%     end
    %----------------------------------------- 
    % Thresholding
    ...
    switch option.algo
        case 'svmclass'
%             [grad] = gradsvmclass4MKL(K,Alpsup,C,yapp,option);
            [grad] = gradsvmclass4MKL(K,Alpsup,Alpsup2,C,yapp,option);
        case 'svmreg'
            grad = gradsvmreg(K,Alpsup,yapp,option) ;
    end;
% 
